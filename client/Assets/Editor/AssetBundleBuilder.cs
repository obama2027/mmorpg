...
public static class AssetBundleBuilder
{
    ...
    private static void BuildInternal(BuildTarget target, bool cleanOutput)
    {
        ...
        GenerateVersionJson(outputPath, target);
        AssetDatabase.Refresh();
    ...
    private static void CleanupStaleBundleFiles(string outputPath, BuildTarget target)
    {
        ...
        for (var i = 0; i < files.Length; i++)
        {
            ...
            if (fileName.EndsWith(".dll", System.StringComparison.OrdinalIgnoreCase) ||
                fileName.EndsWith(".dll.bytes", System.StringComparison.OrdinalIgnoreCase) ||
                fileName.Equals(VersionJsonFileName, System.StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
    ...
    /// <summary>
    /// Updates version.json with dll entries (base.dll.bytes, game.dll.bytes).
    /// Called by BuildResWindow after CopyHotUpdateDllsToStreamingAssets.
    /// Preserves existing bundles if version.json exists.
    /// </summary>
    public static void UpdateVersionJsonWithDlls(string platformDirFullPath)
    {
        var manifest = ReadOrCreateVersionJson(platformDirFullPath);
        manifest.Dlls = new List<VersionJsonDllEntry>();
        manifest.Bundles = manifest.Bundles ?? new List<VersionJsonBundleEntry>();

        foreach (var fileName in new[] { "base.dll.bytes", "game.dll.bytes" })
        {
            var filePath = Path.Combine(platformDirFullPath, fileName);
            ...
            manifest.Dlls.Add(new VersionJsonDllEntry
            {
                Path = fileName,
                Md5 = CalculateFileMd5(filePath),
                Size = fileInfo.Length,
            });
        }

        manifest.Platform = manifest.Platform ?? Path.GetFileName(platformDirFullPath.TrimEnd(Path.DirectorySeparatorChar));
        manifest.Version = BundleVersion;
        manifest.BuildTime = DateTime.UtcNow.ToString("O");
        WriteVersionJson(platformDirFullPath, manifest);
    }

    private static void GenerateVersionJson(string outputPath, BuildTarget target)
    {
        var platformDirFullPath = Path.GetFullPath(outputPath);
        var manifest = ReadOrCreateVersionJson(platformDirFullPath);
        manifest.Dlls = manifest.Dlls ?? new List<VersionJsonDllEntry>();
        manifest.Bundles = new List<VersionJsonBundleEntry>();
        ...
        for (var i = 0; i < bundleFiles.Length; i++)
        {
            ...
            manifest.Bundles.Add(new VersionJsonBundleEntry
            {
                Path = fileInfo.Name,
        ...
        manifest.Platform = GetPlatformName(target);
        manifest.Version = BundleVersion;
        manifest.BuildTime = DateTime.UtcNow.ToString("O");
        WriteVersionJson(platformDirFullPath, manifest);
    }

    private static VersionJsonManifest ReadOrCreateVersionJson(string platformDirFullPath)
    {
        var filePath = Path.Combine(platformDirFullPath, VersionJsonFileName);
        if (!File.Exists(filePath))
        {
            return new VersionJsonManifest();
        }

        try
        {
            var json = File.ReadAllText(filePath, Encoding.UTF8);
            var manifest = JsonUtility.FromJson<VersionJsonManifest>(json);
            return manifest ?? new VersionJsonManifest();
        }
        catch
        {
            return new VersionJsonManifest();
    ...
    private static void WriteVersionJson(string platformDirFullPath, VersionJsonManifest manifest)
    {
        var filePath = Path.Combine(platformDirFullPath, VersionJsonFileName);
